...
dtVarWCC <- dtVarImp[tier %in% c("main") & response == "woody_cover_trend_venter2019", ]
dtVarWCC$clean_term <- factor(dtVarWCC$clean_term, levels = oTWCC)


pVarWCC <- ggplot(data = dtVarWCC) +
  geom_col(aes(y = clean_term, x = mean_rel_imp, fill = responseGroup, group = tier),
           position = position_dodge(width = 1), alpha = 0.9) +
  geom_errorbar(aes(y = clean_term, 
                    xmin = mean_rel_imp - sd_rel_imp, 
                    xmax = mean_rel_imp + sd_rel_imp,
                    group = tier),
...
dtMargWCC <- dtMarg %>% 
  ...
  filter(term %in% c("n_deposition", "herbi_fun_div_distq1")) 
...
pdpWCC <- ggplot()+
  ...
  scale_x_continuous(breaks = extended_breaks(n = 3)) +
  scale_color_manual(values = palette.groups) +
  geom_rug(data = dtRugWCC, aes(x = x, y = y), sides="b", length = unit(0.015, "npc"), outside = TRUE) +
  coord_cartesian(clip = "off") +
...
ggsave(plot = pdpWCC, filename = "builds/presentationPlots/twoTiersWCC.png", dpi = 600, height = 4, width = 8)
...
dtMargWCSD <- dtMarg %>% 
  ...
  filter(term %in% c("browser_biomass_kgkm2","grazer_biomass_kgkm2",
                     "herbi_fun_div_distq1", "CW_mean_species_body_mass")) 
...
pdpWCSD <- ggplot()+
  ...
  scale_x_continuous(breaks = extended_breaks(n = 3)) +
  scale_color_manual(values = palette.groups) +
  geom_rug(data = dtRugWCSD, aes(x = x, y = y), sides="b", length = unit(0.015, "npc"), outside = TRUE) +
  coord_cartesian(clip = "off") +
...
ggsave(plot = pdpWCSD, filename = "builds/presentationPlots/twoTiersWCSD.png", dpi = 600, height = 3.25, width = 12)
