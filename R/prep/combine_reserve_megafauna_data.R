...
dt.sanp <- fread("data/data_fragments/sensitive_data_fragments/sanparks_cleanish.csv")
sf.sanp <- st_read("data/data_fragments/sensitive_data_fragments/sanparks_shapes.gpkg")
...
## HC 
dt.hc <- fread("data/data_fragments/sensitive_data_fragments/hc_cleanish.csv")
names(dt.hc)
sf.hc <- st_read("data/data_fragments/sensitive_data_fragments/hc_shapes.gpkg") 
...
## EWT 
dt.ewt <- fread("data/data_fragments/sensitive_data_fragments/ewt_cleanish.csv")
names(dt.ewt)
sf.ewt <- st_read("data/data_fragments/sensitive_data_fragments/ewt_shapes.gpkg")
...
dt.swep <- fread("data/data_fragments/sensitive_data_fragments/swep_cleanish.csv")
names(dt.swep)
sf.swep <- st_read("data/data_fragments/sensitive_data_fragments/swep_shapes.gpkg")
...
sf_use_s2(FALSE)
#SWEP is the most recent and they came w actual borders, so let's keep them when in doubt 
...
dt.cov4 <- dt.cov3 %>% dplyr::select(- c(hempson_fg, trophic_groups, carnivore_fg_own, herbivore_fg_own, 
                                         ...
                                         prescribed_fire, clear_veg, mean_rainfall, one_per_row)) %>%
  dplyr::select(-reserve_name_new, -reserve_name)
