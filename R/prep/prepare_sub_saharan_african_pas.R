...
all_pas <- st_read("data/spatial/pa_shapes/african_pas.gpkg") ## output from script: "get_african_pas.R".
unique(all_pas$IUCN_CAT)
africa <- ne_countries(continent = "Africa", type = "map_units")

dt_desig <- as.data.frame(table(all_pas$DESIG_ENG)) %>% arrange(-Freq)

dt_desig
#remove the biosphere reserves
strict_pas_raw <- all_pas %>% filter(!grepl("Biosphere Reserve", DESIG_ENG) 
...
## output from script: "prepare_sa_pas"
sa_pa_shp <- st_read("data/spatial/pa_shapes/south_african_pas.gpkg") %>% dplyr::select(NAME, WDPA_PID, DESIG_ENG, GIS_AREA)

missing <- sa_pa_shp %>% filter(!WDPA_PID %in% alin) 


strict_pas <- rbind(strict_pas_raw, missing)
nrow(strict_pas %>% filter(WDPA_PID %in% sa_pa_ids))
table(strict_pas$DESIG_ENG)
...
strict_pas$spatial_predictor1 <- mems2[,1]
strict_pas$spatial_predictor2 <- mems2[,2]
strict_pas$spatial_predictor3 <- mems2[,3]
strict_pas$spatial_predictor4 <- mems2[,4]
...
# 
# 
# mat.extr <- get.heterogeneity(vector = strict_pas, grid = NULL, id.col = "WDPA_PID", raster = mat)
...
# plot(map)
# 
# map.extr <- get.heterogeneity(vector = strict_pas, grid = NULL, id.col = "WDPA_PID", raster = map)
...
# 
# 
# fir.extr <- get.heterogeneity(vector = strict_pas, grid = NULL, id.col = "WDPA_PID", raster = fir)
...
# plot(ndep)
# 
# ndep.extr <- get.heterogeneity(vector = strict_pas, grid = NULL, id.col = "WDPA_PID", raster = ndep)
...
# 
# 
# dsf.burned.extr <- get.heterogeneity(vector = strict_pas, grid = NULL, id.col = "WDPA_PID", raster = dsf.burned)
# setnames(dsf.burned.extr, c("mean"), 
#          c("prop_burned_area"))
# 
# 
# ### import GEE stuff
# venter.trend_raw <- fread("data/data_fragments/VenterWoodyStatsMeanAfricanPas.csv") %>% 
...
# 
# sa.venter_raw <- fread("data/south_african_pas_w_covariates.csv") %>% dplyr::select(WDPA_PID, woody_trend_venter2019) %>% mutate(WDPA_PID = as.character(WDPA_PID))
# 
# sa.venter <-  sa.venter_raw %>% filter(!WDPA_PID %in% c(unique(venter.trend_raw$WDPA_PID)))
# 
# venter.trend <- rbind(sa.venter, venter.trend_raw) %>% unique()
# n_distinct(venter.trend$WDPA_PID)
# 
# quantile(venter.trend$woody_trend_venter2019)
# 
# 
# pa.cov.0.1 <- strict_pas %>% 
#   as.data.table() %>% 
...
# 
# nrow(pa.cov.0.1 %>% filter(WDPA_PID %in% sa_pa_ids))
# 
# pa.cov.shp <- pa.cov.0.1 %>% left_join(strict_pas[, c("NAME", "WDPA_PID", "GIS_AREA")]) %>% st_as_sf()
# 
# ## spatial predictors: ------
# library(spatialRF)
# 
...
# pa.cov.shp <- pa.cov %>% left_join(strict_pas[, c("NAME", "WDPA_PID", "GIS_AREA")]) %>% st_as_sf()
