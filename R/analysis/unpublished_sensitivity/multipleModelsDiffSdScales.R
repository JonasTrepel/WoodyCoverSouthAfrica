...
dt.het <- fread("data/data_fragments/larger_scale_tc_sd.csv")


dt <- fread("data/ReserveDataSouthAfricaFinal.csv")  %>% 
  mutate(
    Biome = as.factor(Biome), 
    elephant_yn = ifelse(elephant_biomass_ha > 0, "elephants", "no_elephants")) %>% left_join(dt.het)
...
responses <- c("tree_cover_sd_50", "tree_cover_sd_100", "tree_cover_sd_100_500", "tree_cover_sd_100_1k")
...
tier.resp <- CJ(response = responses, tier = tiers) %>% 
  mutate(terms = case_when(
    response == "tree_cover_sd_50" ~ "'MAT', 'MAP', 'elevation_sd_1000', 'CW_mean_species_body_mass', 'herbi_fun_div_distq1', 'n_herbi_sp_reserve', 'grazer_biomass_kgkm2', 'browser_biomass_kgkm2', 'mixed_feeder_biomass_kgkm2', 'herbi_biomass_kgkm2', 'fire_events_since_2001', 'prop_burned_area'",
    ...
    response == "tree_cover_sd_100_500" ~ "'MAT', 'MAP', 'elevation_sd_1000', 'CW_mean_species_body_mass', 'herbi_fun_div_distq1', 'n_herbi_sp_reserve', 'grazer_biomass_kgkm2', 'browser_biomass_kgkm2', 'mixed_feeder_biomass_kgkm2', 'herbi_biomass_kgkm2', 'fire_events_since_2001', 'prop_burned_area'",
    response == "tree_cover_sd_100_1k" ~ "'MAT', 'MAP', 'elevation_sd_1000', 'CW_mean_species_body_mass', 'herbi_fun_div_distq1', 'n_herbi_sp_reserve', 'grazer_biomass_kgkm2', 'browser_biomass_kgkm2', 'mixed_feeder_biomass_kgkm2', 'herbi_biomass_kgkm2', 'fire_events_since_2001', 'prop_burned_area'"
  )) %>% mutate(
    response_tier = paste0(response, "_", tier)
  ) %>% 
  mutate(terms = case_when(
    .default = terms,
    grepl("elephants", response_tier) ~ "MAT, MAP, CW_mean_species_body_mass, herbi_fun_div_distq1, n_herbi_sp_reserve, elephant_biomass_kgkm2, herbi_biomass_kgkm2, fire_events_since_2001",
  ))
...
for(i in 1:nrow(dt.tier)){
  ...
  clean.label <- case_when(
    .default = response, 
    response == "tree_cover_sd_50" ~ "Woody cover heterogeneity (10m pixel at 0.25ha)", 
    response == "tree_cover_sd_100" ~ "Woody cover change  (10m pixel at 1 ha)",
    response == "tree_cover_sd_100_500" ~ "Woody cover heterogeneity  (100m pixel at 0.25km2)", 
    response == "tree_cover_sd_100_1k" ~ "Woody cover heterogeneity  (100m pixel at 1km2)")
  ...
  filename.comp <- paste0("builds/plots/july/tc_sd_different_scales/comparison_plot_", response_tier, ".png")
  ...
  filename <- paste0("builds/plots/july/tc_sd_different_scales/comb_plot_", response_tier, ".png")
