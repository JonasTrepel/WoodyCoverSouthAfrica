...
guide.subset <- "response %in% c('tree_cover_mean') & tier %in% c('main', 'high_biomass', 'old', 'large', 'savanna', 'albany_thicket', 'grassland', 'fynbos', 'nama_karoo')"
...
for(i in 1:nrow(dt.tier)){
  ...
  dt.gbm <- dt.sub %>% 
    dplyr::select(all_of(response), all_of(terms.vector)) %>% dplyr::filter(complete.cases(.)) # %>% sample_n(30)
  ...
  if(response == "tree_cover_mean"){
    ggsave(plot = p.comp, filename = filename.comp, dpi = 600, height = 5, width = 13)
  }else{
