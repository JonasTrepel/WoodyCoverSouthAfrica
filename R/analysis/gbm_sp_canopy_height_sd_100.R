...
foreach.results <- foreach(i = 1:nrow(dt.tier),
                           ...
                           .combine = rbind) %dopar% {
                             ...
                             tic()
                             gbmFit <- train(`Canopy height heterogeneity` ~ .,
                                             data = dt.gbm, 
                                             method = "gbm",
                             ...
                             filename.final.mod <- paste0("builds/models/july/gbms/canopy_height_sd_100/", tier, ".rds")
                             ...
                             for(number in 1:100){
                               ...
                               Index <- createDataPartition(dt.gbm$`Canopy height heterogeneity`, p =0.8, list = F)
                               Train <- dt.gbm[Index,]
                               Test <- dt.gbm[!Index,]
                               
                               
                               fit <- gbm(`Canopy height heterogeneity` ~ ., 
                                          data = Train, 
                               ...
                               pr <- caret::postResample(pred = predict(fit, newdata = Test),
                                                         obs = Test$`Canopy height heterogeneity`)
                             ...
                             filename.psi <- paste0("builds/plots/july/gbm_res_sp/canopy_height_sd_100/psi_plot_", tier, ".png")
                             ...
                             p.pd.final <- ggplot()+
                               ...
                               theme_bw() +
                               labs(y = "Canopy height heterogeneity", x = "") +
                               theme(legend.position = "none",
                             ...
                             ##save comb plot 
                             filename <- paste0("builds/plots/july/gbm_res_sp/canopy_height_sd_100/comb_plot_", tier, ".png")
...
fwrite(foreach.results, "builds/model_results/model_meta_canopy_height_sd_100.csv")
stop(pb)
