...
foreach.results <- foreach(i = 1:nrow(dt.tier),
                           ...
                           .combine = rbind) %dopar% {
                             ...
                             if(!tier == "elephants"){
                               ...
                               setnames(dt.gbm, c("woody_cover_trend_venter2019", 
                                                  "MAT", "MAP", "n_deposition",
                                                  "CW_mean_species_body_mass", 
                                                  "herbi_fun_div_distq1", "n_herbi_sp_reserve", 
                                                  "grazer_biomass_kgkm2", "browser_biomass_kgkm2", "mixed_feeder_biomass_kgkm2", "herbi_biomass_kgkm2",
                                                  "fire_events_since_2001", "prop_burned_area"), 
                                        c("Woody cover trend", "MAT (°C)", "MAP (mm)", "N deposition ((kg/km2)/year)",  "Mean body mass (kg; cwm)",
                             ...
                             tic()
                             gbmFit <- train(`Woody cover trend` ~ .,
                                             data = dt.gbm, 
                                             method = "gbm",
                             ...
                             filename.final.mod <- paste0("builds/models/july/gbms/woody_cover_trend/", tier, ".rds")
                            # saveRDS(final.mod, file = filename.final.mod)
                             ...
                             for(number in 1:100){
                               ...
                               Index <- createDataPartition(dt.gbm$`Woody cover trend`, p =0.8, list = F)
                               Train <- dt.gbm[Index,]
                               Test <- dt.gbm[!Index,]
                               
                               
                               fit <- gbm(`Woody cover trend` ~ ., 
                                          data = Train, 
                             ...
                             filename.psi <- paste0("builds/plots/july/gbm_res_sp/woody_cover_trend/psi_plot_", tier, ".png")
                             ...
                             if(!tier == "elephants"){
                               dt.points <- dt.gbm %>% pivot_longer(
                                 cols = c("MAT (°C)", "MAP (mm)", "N deposition ((kg/km2)/year)",  "Mean body mass (kg; cwm)",
                                          "Herbivore functional diversity", "Herbivore species richness", 
                                          "Grazer biomass (kg/km2)", "Browser biomass (kg/km2)", "Mixed feeder biomass (kg/km2)",
                                          "Herbivore biomass (kg/km2)",  "Fire frequency", "Proportion burned area"), 
                             ...
                             ##save comb plot 
                             filename <- paste0("builds/plots/july/gbm_res_sp/woody_cover_trend/comb_plot_", tier, ".png")
...
fwrite(foreach.results, "builds/model_results/model_meta_woody_cover_trend_venter2019.csv")
stop(pb)
