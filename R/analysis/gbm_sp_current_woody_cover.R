...
subsets <- c("!is.na(woody_cover_mean_venter2019)", "elephant_yn == 'elephants'", "fire_events_since_2001 > 0", 
             ...
             "Biome == 'Nama-Karoo'", "elevation_sd_1000 > 17.5")
...
tiers <- c("main", "elephants", "fire", "high_biomass", "low_biomass", "old", "large", 
           "savanna", "albany_thicket", "grassland", "fynbos", "nama_karoo", "high_elevation_sd")
dt.tier <- data.table(
  subset = subsets, 
  tier = tiers, 
...
foreach.results <- foreach(i = 1:nrow(dt.tier),
                           ...
                           .combine = rbind) %dopar% {
                             ...
                             if(!tier == "elephants"){
                               dt.gbm <- dt.sub %>% 
                                 dplyr::select(tree_cover_mean, 
                                               MAT, MAP,  
                                               CW_mean_species_body_mass,  herbi_biomass_kgkm2,
                                               fire_events_since_2001, prop_burned_area, spatial_predictor1)
                               setnames(dt.gbm, c("tree_cover_mean", 
                                                  "MAT", "MAP",
                                                  "CW_mean_species_body_mass",  "herbi_biomass_kgkm2",
                                                  "fire_events_since_2001", "prop_burned_area"), 
                                        c("Current woody cover", "MAT (°C)", "MAP (mm)",   "Mean body mass (kg; cwm)",
                               ...
                               dt.gbm <- dt.sub %>% 
                                 dplyr::select(tree_cover_mean, 
                                               MAT, MAP, 
                                               CW_mean_species_body_mass, elephant_biomass_kgkm2, herbi_biomass_kgkm2,
                             ...
                             tic()
                             gbmFit <- train(`Current woody cover` ~ .,
                                             data = dt.gbm, 
                                             method = "gbm",
                             ...
                             filename.final.mod <- paste0("builds/models/july/gbms/woody_cover_mean/", tier, ".rds")
                             ...
                             #### SET 100 for final run 
                             r.sq <- data.table(r_squared = NA, rmse_bootstrap = NA) %>% filter(!is.na(r_squared))
                             bt.rel <- data.table()
                             marg.plot.bt <- data.table()
                             for(number in 1:100){
                               
                               Index <- createDataPartition(dt.gbm$`Current woody cover`, p =0.8, list = F)
                               Train <- dt.gbm[Index,]
                               Test <- dt.gbm[!Index,]
                               
                               
                               fit <- gbm(`Current woody cover` ~ ., 
                                          data = Train, 
                             ...
                             filename.psi <- paste0("builds/plots/july/gbm_res_sp/woody_cover_mean/psi_plot_", tier, ".png")
                             ...
                             if(!tier == "elephants"){
                               dt.points <- dt.gbm %>% pivot_longer(
                                 cols = c(`MAT (°C)`, `MAP (mm)`, `Mean body mass (kg; cwm)`, `Proportion burned area`,
                             ...
                             ##save comb plot 
                             filename <- paste0("builds/plots/july/gbm_res_sp/woody_cover_mean/comb_plot_", tier, ".png")
                             ggsave(plot = p.comb, filename = filename, dpi = 600, height = 4.5, width = 13)
...
fwrite(foreach.results,  "builds/model_results/model_meta_tree_cover_mean.csv")
stop(pb)
